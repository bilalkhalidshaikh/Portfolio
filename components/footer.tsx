...
export default function Footer() {
  ...
  return (
    <footer ref={ref} className="relative z-[999]">
      {/* Sleek, Floating Minimalist Footer */}
      <motion.div
        style={{ y }}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        onHoverStart={() => setFooterOpen(true)}
        onHoverEnd={() => setFooterOpen(false)}
        className={`fixed bottom-6 left-6 z-[999] backdrop-blur-2xl border border-white/10 bg-[#050505]/80 shadow-[0_10px_40px_rgba(0,0,0,0.5)] transition-all duration-500 ease-[0.16,1,0.3,1] rounded-full overflow-hidden flex items-center ${
          footerOpen
            ? "px-6 py-3 w-auto"
            : "px-0 w-[44px] h-[44px] justify-center"
        }`}
      >
        {footerOpen ? (
          <div className="flex items-center gap-6">
            <p className="text-xs font-mono text-zinc-400 whitespace-nowrap flex items-center gap-2 uppercase tracking-widest">
              <MdCopyright className="text-sm" />
              {new Date().getFullYear()} // BILAL KHALID
            </p>
